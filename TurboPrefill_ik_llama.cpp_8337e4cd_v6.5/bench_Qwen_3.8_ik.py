...
CONFIG_NAME = "config_Qwen_3.8_ik.sh"
...
def load_cfg():
    ...
    defaults = {
        ...
        "OUTPUT_DIR": str(RUN_DIR / "bench_reports_Qwen3.8-27B-Q8_0_ik"),
        "HOST": "0.0.0.0",
        "PORT": "8081",
        "NGL": "999",
        "CTX_SIZE": "200000",
...
def make_server_env():
    ...
    if cfg.get("GGML_SCHED_DEBUG", "0") != "0":
        env["GGML_SCHED_DEBUG"] = cfg["GGML_SCHED_DEBUG"]
    else:
        env.pop("GGML_SCHED_DEBUG", None)
...
def build_server_command():
    ...
    # ik_llama.cpp text-only mode is the default when --mmproj is omitted.

    spec_type = cfg.get("SPEC_TYPE", "none").strip()
    if spec_type and spec_type.lower() != "none":
        spec_type_lower = spec_type.lower()
        if spec_type_lower in {"mtp", "draft-mtp"}:
            # ik_llama.cpp uses a stage-local MTP specification.
            spec_arg = "mtp"
            if cfg.get("SPEC_DRAFT_N_MAX", "").strip():
                spec_arg += f":n_max={cfg['SPEC_DRAFT_N_MAX'].strip()}"
            cmd += ["--spec-type", spec_arg]
        else:
            cmd += ["--spec-type", spec_type]
            if cfg.get("SPEC_DRAFT_N_MAX", "").strip():
                cmd += ["--draft-max", cfg["SPEC_DRAFT_N_MAX"].strip()]

        if cfg.get("SPEC_DRAFT_MODEL", "").strip():
            cmd += ["--model-draft", cfg["SPEC_DRAFT_MODEL"].strip()]
