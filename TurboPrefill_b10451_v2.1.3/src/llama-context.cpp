    ...
    if (turboprefill_enabled && n_outputs_all > 1) {
        ...
        for (uint32_t i = 0; i < n_tokens_all && turboprefill_enabled; ++i) {
            ...
            for (int32_t s = 0; s < ns; ++s) {
                ...
                if (seq_id < 0 || (uint32_t) seq_id >= n_seq_max || seq_has_output[seq_id] != 0) {
    ...
    do {
        ...
        // only meaningful in LLAMA_POOLING_TYPE_NONE (per-token); other pooling modes are ignored.
        {
            ...
            if (embd_nextn.data && t_h_nextn && n_rows > 0 && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
                ...
                const size_t nextn_size = n_rows*n_embd*sizeof(float);
                ggml_backend_tensor_get_async(backend_h, t_h_nextn, embd_nextn_out, 0, nextn_size);

                if (turboprefill.stage > 0 &&
                        !ggml_backend_sched_turboprefill_register_tensor_get(
                            sched.get(), t_h_nextn, embd_nextn_out, 0, nextn_size)) {
                    LLAMA_LOG_WARN("%s: could not register deferred TurboPrefill h_nextn read; keeping the original read\n", __func__);
                }
            }
        }
