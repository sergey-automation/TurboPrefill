...
static enum ggml_status ggml_backend_sched_compute_splits_turboprefill(ggml_backend_sched_t sched, int turboprefill_stage) {
    ...
    // Stage 0 is not TurboPrefill. Keep every fresh scheduler feature on the native path.
    ...
    auto run_turboprefill_one_split_diagonal = [&](
        ...
        if (phase == TP_PHASE_PREPARE_H2D) {
            const auto & snap = saved.split_graphs[split_id];
            if (snap.input_copies.size() != (size_t) splits[split_id].n_inputs) {
                return GGML_STATUS_FAILED;
            }
            for (int input_id = 0; input_id < splits[split_id].n_inputs; ++input_id) {
                struct ggml_tensor * saved_input_cpy = snap.input_copies[input_id];
                struct ggml_tensor * current_input_cpy = current_split_input_copy(&splits[split_id], input_id);
                if (saved_input_cpy == nullptr || current_input_cpy == nullptr) {
                    return GGML_STATUS_FAILED;
                }
            }
        }
        ...
        // Split 0 is executed while each UB is captured. Replay starts at split 1.
            ...
            for (int input_id = 0; input_id < split->n_inputs; ++input_id) {
                ...
                switch (plan->source_kind) {
                    ...
                    case GGML_BACKEND_SCHED_TP_INPUT_SOURCE_UB_INTERMEDIATE:
                        load_ok = (split_id == 1 && input_id == 0) || plan->producer_split_id == 0
                                ? load_saved_info_entry(saved, split_id, input, split_backend, input_cpy)
                                : load_saved_intermediate_entry(saved, split_id, input, split_backend, input_cpy);
                        break;
    ...
    if (turboprefill_stage != 0) {
        ...
        saved_ubs.push_back(std::move(captured));
