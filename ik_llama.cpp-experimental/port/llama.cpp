...
static int llama_decode_internal(
    ...
    if (turboprefill_enabled && n_outputs_all > 1) {
        std::vector<int32_t> seq_token_count(cparams.n_seq_max, 0);
    ...
    lctx.turboprefill.begin_batch(turboprefill_enabled, n_tokens_all, cparams.n_ubatch);
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
        ...
        tim1 = ggml_time_us();
#endif
        lctx.turboprefill.stage_for_ubatch(n_tokens, cparams.n_ubatch);
