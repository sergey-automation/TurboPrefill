...
// TurboPrefill_ik_llama_5763a901_v6.3.2 diagnostics: first target-logits hash and top-16 logits.
...
static std::string format_top_logits(llama_context * ctx, const float * logits, int32_t n_vocab) {
    static constexpr size_t n_top_max = 16;
    if (ctx == nullptr || logits == nullptr || n_vocab <= 0) {
        return "[]";
    }
    ...
    for (size_t i = 0; i < n_top; ++i) {
        ...
        const auto & entry = top_logits[i];
        out << "{\"token_id\":" << entry.token_id
            << ",\"piece\":" << json(common_token_to_piece(ctx, entry.token_id, true)).dump()
            << ",\"logit\":" << entry.logit << '}';
    }
    out << ']';
    return out.str();
...
static void log_first_token_debug(const server_slot & slot, llama_context * ctx, int tok_idx) {
    if (ctx == nullptr || tok_idx < 0) {
        return;
    }

    const llama_model * model = llama_get_model(ctx);
    const int32_t n_vocab = model != nullptr ? llama_n_vocab(model) : 0;
    const float * logits = llama_get_logits_ith(ctx, tok_idx);
    if (logits == nullptr || n_vocab <= 0) {
        return;
    }

    const llama_tokens & prompt_text_tokens = slot.prompt_tokens.get_text_tokens();
    const uint64_t prompt_hash = fnv1a64_bytes(
            reinterpret_cast<const uint8_t *>(prompt_text_tokens.data()),
            prompt_text_tokens.size() * sizeof(llama_token));
    const uint64_t target_logits_hash = fnv1a64_bytes(
            reinterpret_cast<const uint8_t *>(logits), (size_t) n_vocab * sizeof(*logits));
    const std::string top16_logits = format_top_logits(ctx, logits, n_vocab);
...
void server_context::process_batch_tokens(int32_t & n_batch) {
    ...
    if (server_speculative_uses_target_features(params_base.speculative)) {
        ...
        for (auto& slot : slots) {
            ...
            if (slot.n_decoded == 0) {
                log_first_token_debug(slot, ctx, tok_idx);
            }
