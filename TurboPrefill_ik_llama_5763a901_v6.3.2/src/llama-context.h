...
struct llama_context {
    ...
    struct llama_turboprefill_state {
        bool enabled = false;
        // 0 = regular scheduler path. Positive values defer full ubatches;
        // a negative value marks the final full ubatch and triggers replay.
        int stage = 0;
        uint32_t ubatch_index = 0;
        uint32_t n_full_ubatches = 0;
        uint32_t turbo_start_ubatch = 0;

        void begin_batch(bool is_enabled, uint32_t actual_full_ubatches) {
            enabled = is_enabled;
            stage = 0;
            ubatch_index = 0;
            turbo_start_ubatch = 0;
            n_full_ubatches = enabled ? actual_full_ubatches : 0;
        }

        int stage_for_ubatch(uint32_t n_tokens, uint32_t n_ubatch, bool first_ubatch_requires_recurrent_init) {
            stage = 0;

            if (!enabled || n_tokens != n_ubatch) {
                return stage;
            }
            if (ubatch_index == 0 && first_ubatch_requires_recurrent_init && n_full_ubatches > 0) {
            ...
            const uint32_t turbo_count = n_full_ubatches - turbo_start_ubatch;
