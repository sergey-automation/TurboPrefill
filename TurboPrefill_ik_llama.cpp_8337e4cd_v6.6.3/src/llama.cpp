...
static size_t llama_output_reserve_split(
        llama_context & lctx,
                 size_t n_outputs,
    ...
    const size_t n_logits_outputs_max = std::max(n_logits_outputs, (size_t) cparams.n_seq_max);
    const size_t n_embd_outputs_max   = std::max(n_embd_outputs,   (size_t) cparams.n_seq_max);

    const auto n_batch = cparams.n_batch;
    const auto n_vocab = hparams.n_vocab;
    const auto n_embd  = llama_output_embd_width(lctx);

    // Logits and MTP hidden-state rows have different lifetime requirements.
    // Keep separate physical capacities so TurboPrefill can retain all MTP rows
    // without reserving a full-vocabulary logits row for every prompt token.
...
static void llama_graph_compute(
    ...
    const auto status = lctx.turboprefill.stage != 0
            ? ggml_backend_sched_graph_compute_async_turboprefill(lctx.sched, gf, lctx.turboprefill.stage)
...
static int llama_decode_internal(
    ...
    const bool turboprefill_batch_candidate = n_tokens_all >= turboprefill_threshold;

    bool turboprefill_requested = false;
    if (turboprefill_batch_candidate) {
    ...
    // Count the output rows requested by the caller before applying the
    // TurboPrefill MTP compaction below.
    ...
    const uint32_t n_outputs_requested = n_outputs;

    // ik_llama marks every MTP prompt token as a logits output so that the
    // full hidden-state sequence is available to the speculative warmup path.
    // TurboPrefill does not need those full-vocabulary rows. Keep one logits
    // row per nominal micro-batch (including the final partial micro-batch),
    // while retaining hidden-state rows for every prompt token. The last prompt
    // token remains an output, so normal sampling after prefill is unchanged.
    std::vector<int8_t> turboprefill_mtp_logits_mask;
    const bool turboprefill_mtp_compact_outputs =
            turboprefill_requested &&
            has_mtp &&
            cparams.mtp_op_type == MTP_OP_NONE &&
            cparams.cb_eval == nullptr &&
            !embd_pooled &&
            batch_all.logits != nullptr &&
            n_ubatch > 0 &&
            n_tokens_all >= 2 * n_ubatch &&
            n_outputs_requested == n_tokens_all &&
            cparams.causal_attn &&
            model.split_mode == LLAMA_SPLIT_MODE_LAYER &&
            llama_get_device_count(model) > 1;

    if (turboprefill_mtp_compact_outputs) {
        turboprefill_mtp_logits_mask.assign(n_tokens_all, 0);
        n_outputs = 0;

        for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
            const uint32_t end_token = std::min<uint32_t>(cur_token + n_ubatch, n_tokens_all);
            turboprefill_mtp_logits_mask[end_token - 1] = 1;
            ++n_outputs;
        }

        batch_all.logits = turboprefill_mtp_logits_mask.data();
    }

    // MTP target prefill still needs one hidden-state row for every token.
    // Only the physical logits capacity is compacted; embedding capacity stays full-size.
    ...
    const size_t reserved_outputs = turboprefill_mtp_compact_outputs
            ? llama_output_reserve_split(lctx, required_outputs, n_outputs, n_outputs_embd)
            : llama_output_reserve(lctx, required_outputs);
    ...
    const uint32_t n_outputs_all = n_outputs;

    auto turboprefill_chunk_size = [&](uint32_t cur_token) -> uint32_t {
        ...
        return can_check && any_diff && has_dup ? 1 : n_tokens;
    };

    // Turbo can only defer the contiguous full-UB prefix. Use the same chunk
    // rule as the execution loop instead of floor(n_tokens / n_ubatch).
    uint32_t n_turbo_ubatches = 0;
    if (turboprefill_requested) {
        for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
            const uint32_t chunk_size = turboprefill_chunk_size(cur_token);
            if (chunk_size != n_ubatch) {
                break;
            }
            ++n_turbo_ubatches;
            cur_token += chunk_size;
        }
    }
    ...
    bool turboprefill_enabled =
        ...
        //    n_outputs_all <= cparams.n_seq_max &&
            (!cparams.embeddings || (cparams.mtp && has_mtp)) &&
            cparams.causal_attn &&
            model.split_mode == LLAMA_SPLIT_MODE_LAYER &&
            llama_get_device_count(model) > 1 &&
            n_turbo_ubatches >= 2;

    if (turboprefill_enabled && n_outputs_all > 1200000) {
        std::vector<uint8_t> seq_has_output(cparams.n_seq_max, 0);

        for (uint32_t i = 0; i < n_tokens_all && turboprefill_enabled; ++i) {
            if (batch_all.logits != nullptr && batch_all.logits[i] == 0) {
                continue;
            }
            const int ns = batch_all.n_seq_id ? batch_all.n_seq_id[i] : 1;

            for (int32_t s = 0; s < ns; ++s) {
                const llama_seq_id seq_id = batch_all.seq_id ? batch_all.seq_id[i][s] : 0;
                if (seq_id < 0 || (uint32_t) seq_id >= cparams.n_seq_max || seq_has_output[seq_id] != 0) {
    ...
    if (turboprefill_batch_candidate) {
        lctx.turboprefill.begin_batch(turboprefill_enabled, n_turbo_ubatches);
 
        LLAMA_LOG_INFO(
                "%s: TurboPrefill requested=%d active=%d "
                "n_tokens=%u n_ubatch=%u full_ubatches=%u "
                "devices=%d split_mode=%d nextn_layers=%d "
                "mtp_op=%d embeddings=%d mtp=%d has_mtp=%d emb_gate=%d "
                "n_outputs=%u n_seq_max=%u causal=%d compact_mtp=%d "
                "n_outputs_embd=%u requested_outputs=%u logits_mib=%.2f embd_mib=%.2f "
                "buf_used_mib=%.2f buf_alloc_mib=%.2f\n",
                __func__,
                (int) turboprefill_requested,
                (int) lctx.turboprefill.enabled,
                n_tokens_all,
                cparams.n_ubatch,
                lctx.turboprefill.n_full_ubatches,
                (int) llama_get_device_count(model),
                (int) model.split_mode,
                (int) hparams.nextn_predict_layers,
                (int) cparams.mtp_op_type,
                (int) cparams.embeddings,
                (int) cparams.mtp,
                (int) has_mtp,
                (int) (!cparams.embeddings || (cparams.mtp && has_mtp)),
                n_outputs_all,
                cparams.n_seq_max,
                (int) cparams.causal_attn,
                (int) turboprefill_mtp_compact_outputs,
                n_outputs_embd,
                n_outputs_requested,
                lctx.logits_size * sizeof(float) / (1024.0 * 1024.0),
                lctx.embd_size   * sizeof(float) / (1024.0 * 1024.0),
                (lctx.logits_size + lctx.embd_size) * sizeof(float) / (1024.0 * 1024.0),
                lctx.buf_output ? ggml_backend_buffer_get_size(lctx.buf_output) / (1024.0 * 1024.0) : 0.0);
        LLAMA_LOG_INFO(
            "%s: TP req=%d active=%d mtp_op=%d cb=%d tokens=%u ub=%u outputs=%u seqmax=%u causal=%d split=%d devices=%d turbo_ub=%u compact_mtp=%d embd_outputs=%u requested_outputs=%u buf_alloc_mib=%.2f\n",
            __func__,
            (int)turboprefill_requested,
            (int)lctx.turboprefill.enabled,
            (int)cparams.mtp_op_type,
            (int)(cparams.cb_eval != nullptr),
            n_tokens_all,
            cparams.n_ubatch,
            n_outputs_all,
            cparams.n_seq_max,
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
        ...
        auto tim1 = ggml_time_us();
#endif
        uint32_t n_tokens = turboprefill_chunk_size(cur_token);
        if (n_tokens == 1 &&
            llm_arch_is_hybrid(model.arch) &&
            std::min(n_ubatch, n_tokens_all - cur_token) > 1 &&
            !warned_qnext_mixed_repeat) {
        ...
        if (turboprefill_batch_candidate) {
            const bool first_ubatch_requires_recurrent_init =
                    lctx.turboprefill.ubatch_index == 0 &&
                    (llama_model_has_recurrent(&lctx.model) || llama_model_is_openpangu(&lctx.model)) &&
                    kv_self.head == 0;
            lctx.turboprefill.stage_for_ubatch(
                    n_tokens, cparams.n_ubatch, first_ubatch_requires_recurrent_init);
        }
                ...
                case LLAMA_POOLING_TYPE_NONE:
                    {
                        ...
                        if (n_outputs_new_embd) {
                            ...
                            const size_t embd_size = n_outputs_new_embd*n_embd_output*sizeof(float);
                            ggml_backend_tensor_get_async(backend_embd, embd, embd_out, 0, embd_size);
                            if (turboprefill_batch_candidate && has_mtp && lctx.turboprefill.stage > 0 &&
                                    !ggml_backend_sched_turboprefill_register_tensor_get(
                                        lctx.sched, embd, embd_out, 0, embd_size)) {
                                LLAMA_LOG_WARN("%s: could not register deferred TurboPrefill MTP embedding read; keeping the original read\n", __func__);
                            }
                        }
                    } break;
        ...
        if (turboprefill_batch_candidate) {
            lctx.turboprefill.finish_ubatch();
        }
        cur_token += n_tokens;
        if (reset_previous || (turboprefill_batch_candidate && lctx.turboprefill.stage < 0)) {
            // The graph was built with a position-zero state reset. Reusing it at later
            // positions, or after Turbo replay, would reuse stale graph state.
            prev.reset();
        }
    ...
    if (turboprefill_batch_candidate) {
        ...
        lctx.turboprefill.finish_batch();
    }
...
struct llama_context * llama_init_from_model(
    ...
    if (cparams.mtp && (hparams.nextn_predict_layers > 0 || model->arch == LLM_ARCH_DEEPSEEK4)) {
        ...
        // Avoid the legacy n_batch x n_vocab MTP preallocation when TurboPrefill
        // will use compact prompt logits. The runtime decode path keeps all MTP
        // hidden-state rows, but only needs one logits row per nominal micro-batch.
        const bool turboprefill_mtp_compact_init =
                llama_env_flag_enabled("TURBOPREFILL") &&
                n_ubatch > 0 &&
                cparams.causal_attn &&
                model->split_mode == LLAMA_SPLIT_MODE_LAYER &&
                llama_get_device_count(*model) > 1;

        const size_t logits_rows = turboprefill_mtp_compact_init
                ? (n_batch + n_ubatch - 1) / n_ubatch
                : n_batch;
        const size_t embd_rows = n_batch;

        const size_t logits_size = n_vocab*logits_rows;
        const size_t embd_size   = n_embd*embd_rows;
        ...
        if (turboprefill_mtp_compact_init) {
            LLAMA_LOG_INFO(
                    "%s: TurboPrefill MTP compact output preallocation: logits_rows=%zu embd_rows=%zu size=%.2f MiB\n",
                    __func__, logits_rows, embd_rows, new_size / (1024.0 * 1024.0));
        }
